import type { Box, Connection, CoordinateExtent, DefaultEdgeOptions, Dimensions, Edge, EdgeMarkerType, ElementData, Elements, FlowElements, GraphEdge, GraphNode, MaybeElement, Node, NodeLookup, Padding, Rect, ViewportTransform, XYPosition, XYZPosition } from '../types';
...
export declare function getOutgoers<N extends Node>(nodeOrId: Node | {
    id: string;
} | string, nodes: N[], edges: Edge[]): N[];
export declare function getOutgoers<T extends Elements>(nodeOrId: Node | {
    id: string;
} | string, elements: T): T extends FlowElements ? GraphNode[] : Node[];
export declare function getIncomers<N extends Node>(nodeOrId: Node | {
    id: string;
} | string, nodes: N[], edges: Edge[]): N[];
export declare function getIncomers<T extends Elements>(nodeOrId: Node | {
    id: string;
} | string, elements: T): T extends FlowElements ? GraphNode[] : Node[];
...
export declare function pointToRendererPoint({ x, y }: XYPosition, { x: tx, y: ty, zoom: tScale }: ViewportTransform, snapToGrid?: boolean, snapGrid?: [snapX: number, snapY: number]): XYPosition;
...
export declare function getNodesInside(nodes: GraphNode[], rect: Rect, viewport?: ViewportTransform, partially?: boolean, excludeNonSelectableNodes?: boolean): GraphNode<any, any, string>[];
 ...
 * 1200, 800, 0.5, 2);
 */
export declare function getTransformForBounds(bounds: Rect, width: number, height: number, minZoom: number, maxZoom: number, padding?: Padding): ViewportTransform;
export declare function getXYZPos(parentPos: XYZPosition, computedPosition: XYZPosition): XYZPosition;
export declare function isParentSelected(node: GraphNode, nodeLookup: NodeLookup): boolean;
